...
begin
	...
	using MITgcm, MeshArrays, Drifters
	using PlutoUI, Dates, JLD2, Plots, ColorSchemes
    ...
    found_MITgcm_files=ispath(pth)
    if found_MITgcm_files
        println("😄 Input files were found in \n $pth")
    else
        println("!!! Necessary files NOT found in \n!!! $pth")
        println("!!! Please make sure to run the `HS94_animation.jl` notebook first")
    end
end
...
begin
	...
	In this notebook, we setup, run and plot MITgcm output via [Drifters.jl](https://juliaclimate.github.io/Drifters.jl/dev/) to generate something like what's shown below. Model output from the `HS94_animation.jl` notebook are used to drive the particle advection model.
	"""
end
...
begin
	...
	md"""$(Resource(imgA, :width => 360))"""
end

# ╔═╡ a6ebb38e-2dcd-4126-bfa1-220df3180b94
md""" ### Julia Packages And Model Files

Model grid and output files are expected to be found in the `HS94_animation_link` folder indicated  below, which the `HS94_animation.jl` notebook has generated.

!!! tip
    Please make sure to run `HS94_animation.jl` first before running this notebook.

$(TableOfContents())
"""

# ╔═╡ f9d76687-5ca8-4f39-8b46-67322866b8db
md"""A list of grid variables should appear afterwards if files are indeed found as expected.
...
md""" ### Setup Computation

- Individuals 𝐼 are those that will be advected by the flow fields using the `∫!` function. 
- This is done in a time loop with `update_FlowFields!` loading flow fields periodically. 
- Here is a summary of the 𝐼 data structure:
"""		
...
if found_MITgcm_files
	...
	𝐷=(pth=pth,k=k,dt=dt,t00=t00,Γ=Γ,
	    ...
	    XC=exchange(Γ.XC).MA,YC=exchange(Γ.YC).MA,
	    iDXC=1. ./Γ.DXC, iDYC=1. ./Γ.DYC)
	
	update_loc=(u -> MeshArrays.update_location_cs!(u,𝐷))
	tmp = MeshArrays.NeighborTileIndices_cs(Γ)
	𝐷 = merge(𝐷 , tmp)
	...
	custom∫(prob) = Drifters.ensemble_solver(
		prob; solver=Drifters.Tsit5(),
		reltol=1e-5,abstol=1e-5,safetycopy=false)
	𝐼=Individuals(𝑃,x,y,f,(∫=custom∫,))
	#𝐼=Individuals(𝑃,x,y,f)
	
	#Function that read velocity fields from 𝐼.P.T[2] and at level=D.k
	function update_FlowFields!(I::Individuals,D::NamedTuple)
	    m0=Int(floor((I.P.T[2]-D.t00)/D.dt))+1
	    m1=m0+1
	    t0=m0*D.dt-D.dt+D.t00
	    t1=m1*D.dt-D.dt+D.t00
	    #println(m0)
...
# ╔═╡ 71a2a81b-835f-4f51-92d8-dace2cd87734
𝐼.P.u0.fSize

# ╔═╡ 5129ac14-0861-40b5-b97e-0d0edd37849f
md""" ### Plot Results

The plots below, generated by `plot_xy` function, show:
...
To save a plot, add somthing like `savefig(tempdir()*"/"*"hs94.cs.particles.png")`
"""

# ╔═╡ e9535b96-07c2-4aff-964d-bb327c5f735b
begin
	"""
	    plot(𝐼::Individuals)
...
if found_MITgcm_files
	...
	add_lonlat!(𝐼.🔴,exchange(Γ.XC),exchange(Γ.YC))

	🔴 = 𝐼.🔴
end

# ╔═╡ a34adf36-81de-42c2-8407-d408041193b6
found_MITgcm_files ? f1 : nothing

# ╔═╡ a1fdaf62-1b80-481e-a5c7-45e82c540368
found_MITgcm_files ? f2 : nothing

# ╔═╡ c473fb90-8901-466e-a8e3-c7efd189e1ba
found_MITgcm_files ? f3=plot_xy(𝐼) : nothing

# ╔═╡ 2d320d61-9e55-471e-b663-8d95f2055b84
md"""### Save Results

The file generated in the next code cell is used in the `HS94_Makie.jl` example.
"""
...
git-tree-sha1 = "422a791699dd3ccd425af047ca4249252aec7cbc"
...
version = "0.6.21"
    ...
    DriftersStochasticDiffEqExt = ["StochasticDiffEq"]

    [deps.Drifters.weakdeps]
...
git-tree-sha1 = "02a81588dfb3b1efc9fef5d9d05463956f6c08c7"
...
git-tree-sha1 = "da227ccbb64503939fa067b169817eec404c04f4"
...
git-tree-sha1 = "882e0c75c79c058d2dc2f20f8b809d26ec032808"
...
git-tree-sha1 = "577f453cd7cf893cef584f7372b78a59e227416d"
...
git-tree-sha1 = "3275d1bd77357116c50ac566d286a79dedd077d6"
...
git-tree-sha1 = "9315d0c76c4411c47f037e5f16f90135abe4607f"
...
git-tree-sha1 = "805c050714d23064eb4f8ee8793604d294938dc0"
...
git-tree-sha1 = "ba754335f4faf255657bbf89cdc65f768800f1e0"
