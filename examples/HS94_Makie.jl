...
begin
	using JLD2, FileIO, DataFrames, CairoMakie, MITgcm
	🏁 = "🏁"
	pth=joinpath(tempdir(),"HS94_animation_link","run")
	fil=joinpath(pth,"HS94.jld2") #generated by HS94_particles.jl

    found_HS94_files=ispath(fil)
    if found_HS94_files
        println("😄 Input files were found in \n $pth")
    else
        println("!!! Necessary files NOT found in \n!!! $pth")
        println("!!! Please make sure to run the `HS94_particles.jl` notebook first")
    end
end
...
begin
	...
	Here we read trajectory output generated in `HS94_particles.jl` (file name below).
	
	Then we plot it using `Makie.jl` package instead of `plots.jl`.
	"""
end
...
if found_HS94_files
    ...
    fi1,ax1 = scatter(lo, la, markersize = 4.0, color=:blue, strokewidth = 0.0)
	xlims!(ax1,(-180.,180.)); ylims!(ax1,(-90.,90.))

	##
	
	lo0 = rec_by_t[1].lon; la0 = rec_by_t[1].lat
	lo1 = rec_by_t[nt].lon; la1 = rec_by_t[nt].lat

    fi2,ax2 = scatter(lo0, la0, markersize = 6.0, color=:blue, strokewidth = 0.0)
    scatter!(ax2,lo1, la1, markersize = 6.0, color=:red, strokewidth = 0.0)
	xlims!(ax1,(-180.,180.)); ylims!(ax1,(-90.,90.))
...
if found_HS94_files
	...
	"file saved : "*pp*"HS94_Makie.mp4, .png"
end

# ╔═╡ c07ad77c-87cb-41f7-9352-3273760fc23e
if found_HS94_files
	using PlutoUI
	fil2=pp*"HS94_Makie.mp4"
	LocalResource(fil2)
end
...
manifest_format = "2.0"
project_hash = "b9f8b7f3eed81305d27ef0411e2fc35ce30aec99"
