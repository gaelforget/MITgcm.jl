...
md"""# Setup & Build Model

$(TableOfContents())

!!! tip
	Selecting a new model configuration typically means recompiling the model. This can take longer than a normal model run due to the one-time cost of compiling the model (see below for more on this).
...
- The model executable `mitcmuv` is generally found in a `build/` subfolder within the selected model configuration.
- If `mitcmuv` is not found there, then it needs to be compiled, before we can then run the model. 
- Once `mitgcmuv` is found, then the executable file name should appear just below.

!!! note
    It should also be noted that compiling the model can take a lot longer than running it for a few time steps (as done in this notebook's sample model runs).
"""
...
You can click on the `Launch Model` button to manually trigger a model run.
This will happen automatically the first time, and then once gain after modifying parameters (see below).
"""

# ╔═╡ 6f618b2c-86bd-11eb-1607-a179a349378e
@bind do_run1 PlutoUI.Button("Launch Model")

# ╔═╡ 6404fddf-3c46-4015-9580-b9159c76b30a
md"""# Analyze Results

Let's look at the model run folder and visualize results.

!!! tip
    The MITgcm can output various results and store them in several types of files (text, binary, or netcdf). For more information about file formats and contents, please refer to the [MITgcm documentation](https://mitgcm.readthedocs.io/en/latest/getting_started/getting_started.html#output-files)
...
$(@bind t_slow PlutoUI.Clock(1.0, true))
"""

# ╔═╡ c7670d00-868c-11eb-1889-4d3ffe621dd2
md"""# Modify & Rerun Model
...
begin
	...
	if !isempty(p_value)
		...
		ClimateModels.git_log_fil(MC,tmpfil,"update parameter file")
	end
...
begin
	do_run1
	do_run2
	isempty(MC.channel) ? put!(MC.channel,MITgcm_launch) : nothing
	launch(MC)
	refresh_plot=true
	md"""Model run for the **$(MC.configuration)** configuration has completed!
...
begin
	...
	if MC.configuration=="advect_xy"||MC.configuration=="advect_xz"
		tmp2=readdir(rundir)
		...
		if MC.configuration=="advect_xz"
			contourf(1e-3*x[:,1],reverse(z[:]),reverse(tmp3,dims=2),xlabel="km",ylabel="m",
				levels=(-0.04:0.02:0.2),leg=:none,c = :terrain)
		else
			contourf(1e-3*x[:,1],1e-3*y[1,:],tmp3,xlabel="km",ylabel="km",
...
manifest_format = "2.0"
project_hash = "bc5fe7b39eb736311e8f747651f0d4ffec20ce41"
